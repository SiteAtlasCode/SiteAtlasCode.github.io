...
st.textContent=  @keyframes atlasSpin{0%{transform:rotate(0)}100%{transform:rotate(360deg)}}   @keyframes atlasSweep{0%{transform:translateX(-110%)}100%{transform:translateX(110%)}}   @keyframes atlasPulse{0%,100%{opacity:.22;transform:scaleX(.96)}50%{opacity:1;transform:scaleX(1)}}   @keyframes atlasBlink{0%,100%{opacity:.55}50%{opacity:1}}   @keyframes atlasFadeIn{0%{opacity:0;transform:translateY(10px) scale(.985)}100%{opacity:1;transform:translateY(0) scale(1)}}   @keyframes atlasFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-6px)}}   @keyframes atlasGlow{0%,100%{filter:drop-shadow(0 0 6px rgba(255,255,255,.18))}50%{filter:drop-shadow(0 0 18px rgba(255,255,255,.34))}}   @keyframes atlasShimmer{0%{background-position:-220% 0}100%{background-position:220% 0}}   @keyframes atlasShake{0%,100%{transform:translateX(0)}20%{transform:translateX(-1px)}40%{transform:translateX(1px)}60%{transform:translateX(-1px)}80%{transform:translateX(1px)}}  ;
...
kill("Network instability detected",60601000,{kind:"flap",flaps:networkState.flaps});
}
};
...
kill("Unauthorized script injection",126060*1000,{kind:"script-injection",src:cleanUrl(node.src||"")});
return;
}
}
}
...
kill("Excessive IP changes",60601000,{kind:"ip-change",changes:networkState.ipChanges});
return 999;
}
}
}
...
const box=overlay("atlas-sec-human",  position:fixed;inset:0;display:flex;align-items:center;justify-content:center;   background:   radial-gradient(circle at 20% 20%, rgba(255,255,255,.08), transparent 32%),   radial-gradient(circle at 80% 0%, rgba(255,255,255,.04), transparent 26%),   rgba(5,5,5,.96);   backdrop-filter:blur(12px);z-index:2147483647;font-family:system-ui,sans-serif;color:#fff;padding:20px;  );
box.innerHTML="";
const card=css("div",  width:min(480px,92vw);   background:linear-gradient(180deg, rgba(20,20,20,.98), rgba(10,10,10,.98));   border:1px solid rgba(255,255,255,.08);   border-radius:26px;   box-shadow:0 22px 80px rgba(0,0,0,.62), inset 0 1px 0 rgba(255,255,255,.04);   padding:18px;   display:grid;   gap:14px;   animation:atlasFadeIn .18s ease-out;  );
...
const frame=css("div",  border-radius:20px;overflow:hidden;background:#0f0f0f;border:1px solid rgba(255,255,255,.08);   box-shadow:inset 0 0 0 1px rgba(255,255,255,.02);  );
...
const input=css("input",  width:100%;box-sizing:border-box;background:#fff;color:#111;border:0;border-radius:16px;   padding:14px 16px;font-size:16px;font-weight:800;outline:none;   box-shadow:0 10px 28px rgba(0,0,0,.18);  );
...
const submit=css("button",  border:0;background:#fff;color:#111;padding:13px 18px;border-radius:14px;font-size:15px;   font-weight:900;cursor:pointer;box-shadow:0 0 16px rgba(255,255,255,.14);  );
submit.type="button"; submit.textContent="تأیید";
const refresh=css("button",  border:1px solid rgba(255,255,255,.14);background:transparent;color:#fff;padding:13px 18px;   border-radius:14px;font-size:15px;font-weight:800;cursor:pointer;  );
...
if(hidden.length){
kill("Hidden captcha field filled",126060*1000,{kind:"honeypot"});
resolve(false);
return;
}
...
ensureStyle();
clearOverlays();
const box=overlay("atlas-sec-audit",  position:fixed;inset:0;display:flex;align-items:center;justify-content:center;   background:   radial-gradient(circle at 20% 20%, rgba(255,255,255,.08), transparent 32%),   rgba(4,4,4,.95);   backdrop-filter:blur(10px);   z-index:2147483647;color:#fff;font-family:system-ui,sans-serif;padding:20px;  );
box.innerHTML="";
const card=css("div",  width:min(560px,94vw);   background:linear-gradient(180deg, rgba(18,18,18,.98), rgba(10,10,10,.98));   border:1px solid rgba(255,255,255,.08);   border-radius:26px;   box-shadow:0 22px 80px rgba(0,0,0,.62);   padding:18px;   display:grid;   gap:12px;   animation:atlasFadeIn .18s ease-out;  );
...
if(!ban) return;
STATE.blocked=true;
const box=overlay("atlas-sec-ban",  position:fixed;inset:0;display:flex;align-items:center;justify-content:center;   background:   radial-gradient(circle at 20% 20%, rgba(255,255,255,.08), transparent 32%),   radial-gradient(circle at 80% 80%, rgba(255,255,255,.05), transparent 24%),   rgba(0,0,0,.96);   backdrop-filter:blur(12px);   z-index:2147483647;color:#fff;font-family:system-ui,sans-serif;padding:22px;  );
box.innerHTML="";
const card=css("div",  width:min(620px,95vw);   background:linear-gradient(180deg, rgba(22,22,22,.98), rgba(12,12,12,.98));   border:1px solid rgba(255,255,255,.09);   border-radius:28px;   box-shadow:0 22px 80px rgba(0,0,0,.66), inset 0 1px 0 rgba(255,255,255,.04);   padding:22px;   display:grid;   gap:14px;   animation:atlasFadeIn .18s ease-out;  );
...
const btn=css("button",  border:0;background:#fff;color:#111;padding:13px 18px;border-radius:14px;font-size:15px;   font-weight:900;cursor:pointer;box-shadow:0 0 16px rgba(255,255,255,.14);  );
...
btn.style.display=cur.durationMs===126060*1000?"inline-flex":"none";
};

btn.addEventListener("click",async()=>{
const cur=verifyCurrentBan();
if(!cur||cur.durationMs!==1260601000) return;
btn.disabled=true;
btn.textContent="در حال بررسی...";
const audit=await auditAll({showAudit:false});
if(audit.clear){
const rem=now()+460601000;
storeWrite(K.ban,{...cur,expires:rem,durationMs:46060*1000,reason:${cur.reason} | recheck approved});
...
kill("Too many disabled config entries",126060*1000,{kind:"config-false",count:configFalseCount});
out.reason="config-false";
return out;
}

if(!hostnameOk()){
kill("Invalid domain",126060*1000,{kind:"domain"});
out.reason="domain";
return out;
}

if(!verifySameScriptSrc()){
kill("Tools.js source mismatch",126060*1000,{kind:"src-mismatch",src:SELF_SRC});
...
if(!ok){
kill("Tools.js integrity mismatch",126060*1000,{kind:"integrity"});
...
repairConfigIfNeeded();
kill("Too many disabled config entries",126060*1000,{kind:"config-false"});
return;
}
...
STATE.verified=await captchaGate();
if(!STATE.verified){
kill("Verification required",10601000,{kind:"captcha-fail"});
return;
}
}
